...
import { contentReveal, contentRevealReduced } from "@/lib/motion";
 ...
 * - reveals main content with fade + scale (strongest beat after the gate)
...
export function InvitationExperience({ children }: Props) {
  ...
  return (
    <>
      ...
      <motion.div
        variants={reduced ? contentRevealReduced : contentReveal}
        initial="sealed"
        animate={contentState}
        className="min-h-dvh will-change-[opacity,transform]"
        style={{
